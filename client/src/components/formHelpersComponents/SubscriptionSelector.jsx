	...
	return (
		<FormControl isRequired isInvalid={errors.subscriptionPlan}>
			...
			<Controller
				...
				control={control}
				defaultValue={valueProps || selectedPlan} /// ustawicn default value!!!
							...
							{subscriptionPlans.map(option => {
								...
								return (
									<VStack
										...
										h={100}
										justify='center'
										borderColor={value === option.name || valueProps === option.name ? 'brand.500' : 'brand.100'}
										borderRadius={10}
										_hover={{ borderColor: 'brand.500' }}
										cursor='pointer'
										onClick={() => {
											onChangePlan(option.name)
			...
			/>
			{errors && <FormErrorMessage>{errors.subscriptionPlan && errors.subscriptionPlan.message}</FormErrorMessage>}
		</FormControl>
	)
}
